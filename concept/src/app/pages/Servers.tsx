...
import { Plus, Search, Server, Play, Square, RotateCw, Cpu, HardDrive, Users } from 'lucide-react';
...
export function Servers() {
  ...
  const filteredServers = mockServers.filter(server => {
    ...
    const matchesNamespace = namespaceFilter === 'all' || server.namespaceName === namespaceFilter;
    return matchesSearch && matchesStatus && matchesNamespace;
  });

  const uniqueNamespaces = Array.from(new Set(mockServers.map(s => s.namespaceName)));
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold mb-2">Servers</h1>
            <p className="text-slate-400">Manage your containerized Minecraft servers</p>
          </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <Link to={`/namespace/${server.namespaceId}/servers/${server.id}`} className="text-lg font-semibold hover:text-blue-400">
                        {server.name}
                      </Link>
                    </div>
